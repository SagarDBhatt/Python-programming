...
#
# Sample Input provided in hacker Rank
...
def print_formatted(number):
    # print("Provided number = " + str(n))

    for i in range(1, number + 1):
        # print(i + " " + oct(i) + " " + hex(i).upper() + " " + bin(i))
        width = len(bin(number)[2:])
        print(str(i).rjust(width, ' '), str(oct(i)[2:]).rjust(width, ' '), str(hex(i).upper()[2:]).rjust(width, ' '),str(bin(i)[2:]).rjust(width, ' '))
